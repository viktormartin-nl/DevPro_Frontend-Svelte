<script>
  ...
  onMount(() => {
    ...
    if (urlParams.has("searchText")) {
      ...
      getProfiles(`${BASE_URL}/protected/all-user`);
    }
  });
  ...
  const getProfilesByCategory = (index) => {
    ...
    searchText = "";
    getProfiles(`${BASE_URL}/protected/all-user?title=${indexCategoryProfile}`);
  };

  const getProfilesBySearch = () => {
    searchText = searchText.trim();
    indexCategoryProfile = -1;
    getProfiles(`${BASE_URL}/protected/all-user?skills=${searchText}`);
  };
</script>
...
<section>
  <div class="pb-16 flex flex-col justify-center align-middle">
    ...
    {:else}
      <div class="my-16 text-4xl text-center w-full">No Developers Found</div>
    {/if}
  </div>
</section>
