<script>
    ...
    if (!isAuthenticated()) {
      ...
      axios
        ...
        .then((response) => {
          ...
          userLanguage = userProfile.languages;
          socialLink = {
            fb_url: userProfile.fb_url,
            twitter_url: userProfile.twitter_url,
            linkdin_url: userProfile.linkdin_url,
  ...
  const handleChangeLanguageTime = () => {
    if (userProfile.timezone !== undefined && userProfile.timezone !== null) {
      sendRequest(`${BASE_URL}/protected/update`, {
        timezone: userProfile.timezone,
      });
      console.log(userProfile.timezone);
    }
  ...
  const handleProfilePhoto = () => {
    ...
    if (fileInput.files.length === 0) {
      showToast("Info", "Please select Profile Image", "info");
    } else {
      let formData = new FormData();
      formData.append("profile_photo", fileInput.files[0]);
      axios
        .post(`${BASE_URL}/protected/update-profile-photo`, formData)
        .then((response) => {
  ...
  const handleDeleteProfilePhoto = () => {
    let isAgreed = confirm("Really delete profile image");
    if (isAgreed) {
      axios
        .post(`${BASE_URL}/protected/delete-profile-photo`)
        .then((response) => {
          showToast("Success", response.data.message, "success");
          location.href = "/dashboard/setting";
          console.log(response.data);
  ...
  const handleSubmitPasswordInformation = () => {
    ...
    if (
      passwordInformation.newPassword !== passwordInformation.confirmPassword
    ) {
      showToast("Failed", "Passwords must match", "error");
    } else {
      let requestBody = {
        currentPassword: passwordInformation.currentPassword,
        newPassword: passwordInformation.newPassword,
      };
      axios
        .post(`${BASE_URL}/protected/change-password`, requestBody)
        .then((response) => {
          userProfile = response.data;
          location.href = "/dashboard/setting";
          showToast("Success", "Saved Successfully", "success");
        ...
        .catch((error) => {
          ...
          if (error.response.status === 400) {
            showToast("Failed", error.response.data.errors[0].msg, "error");
  ...
  const handleDisconnectSocial = (social) => {
    console.log(social);
  ...
  const handleConnectSocial = (social) => {
    console.log(social);
    isEditable[social] = true;
  };

  const handleSocialLink = () => {
    console.log(socialLink);
    let isValid = false;
    for (let i = 0; i < Object.keys(socialLink).length; i++) {
      let key = Object.keys(socialLink)[i];
      if (socialLink[key]) {
        isValid = true;
      }
    }
    if (isValid) {
      sendRequest(`${BASE_URL}/protected/update`, socialLink);
    } else {
      showToast("Warning", "Please enter the social link", "warning");
    }
  };
</script>
...
>
  <div class="mb-4 flex flex-col items-start col-span-full xl:mb-2">
    ...
    <h1
      class="text-3xl font-semibold text-gray-900 sm:text-3xl dark:text-white"
    >
      User settings
    </h1>
  </div>
  ...
  <div class="col-span-full xl:col-auto">
    <div
      class="p-4 mb-4 bg-white border border-gray-200 rounded-lg shadow-sm 2xl:col-span-2 dark:border-gray-700 sm:p-6 dark:bg-primary-3"
    >
      <div
        class="items-center sm:flex xl:block 2xl:flex sm:space-x-4 xl:space-x-0 2xl:space-x-4"
      >
        <form on:submit|preventDefault={handleProfilePhoto}>
          <div class="relative">
            <label for="profile_photo">
              {#if userProfile.profile_photo}
                <Img
                  class="mb-4 border-2 rounded-lg w-28 h-28 sm:mb-0 xl:mb-4 2xl:mb-0 cursor-pointer"
            ...
            <input
              ...
              id="profile_photo"
              accept="image/*"
              on:change={handleChangeProfilePhoto}
              class="absolute inset-0 w-28 h-28 opacity-0 cursor-pointer"
            />
          </div>
          <div>
            ...
            <div class="flex items-center space-x-4">
              <button
                class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white rounded-lg bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 dark:bg-primary-1 dark:hover:bg-primary-700 dark:focus:ring-primary-800"
              >
                <svg
              ...
              <button
                type="button"
                on:click={handleDeleteProfilePhoto}
    ...
    <div
      class="p-4 mb-4 bg-white border border-gray-200 rounded-lg shadow-sm 2xl:col-span-2 dark:border-gray-700 sm:p-6 dark:bg-primary-3"
    >
      <form on:submit|preventDefault={handleChangeLanguageTime}>
    ...
    <div
      class="p-4 mb-4 bg-white border border-gray-200 rounded-lg shadow-sm 2xl:col-span-2 dark:border-gray-700 sm:p-6 dark:bg-primary-3"
    >
      <form class="flow-root" on:submit|preventDefault={handleSocialLink}>
            ...
            <div class="flex items-center space-x-4">
              ...
              <div class="flex-1 min-w-0">
                ...
                {#if userProfile.fb_url}
                  <a
                    href={userProfile.fb_url}
                    class="block text-sm font-normal truncate text-primary-700 hover:underline dark:text-primary-1"
                  >
                    {userProfile.fb_url}
                  </a>
                {:else if isEditable["fb_url"]}
                  <input
                    type="text"
                    name="fb_url"
                    id="fb_url"
                    class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                    bind:value={socialLink.fb_url}
              ...
              <div
                class="inline-flex items-center {isEditable.fb_url
                  ? 'hidden'
                  : ''}"
              >
                {#if userProfile.fb_url}
                  <button
                    type="button"
                    on:click={() => handleDisconnectSocial("fb_url")}
                ...
                {:else}
                  <button
                    type="button"
                    on:click={() => handleConnectSocial("fb_url")}
                    class="px-3 py-2 mb-3 mr-3 text-sm font-medium text-center text-white rounded-lg bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 dark:bg-primary-1 dark:hover:bg-primary-700 dark:focus:ring-primary-800"
                  >
                    Connect
                  </button>
                {/if}
          ...
          <li class="py-4">
            <div class="flex items-center space-x-4">
              ...
              <div class="flex-1 min-w-0">
                ...
                {:else if isEditable["twitter_url"]}
                  <input
                    type="text"
                    name="twitter_url"
                    id="twitter_url"
                    class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                    bind:value={socialLink.twitter_url}
              ...
              <div
                class="inline-flex items-center {isEditable.twitter_url
                  ? 'hidden'
                  : ''}"
              >
                {#if userProfile.twitter_url}
                  <button
                    type="button"
                    on:click={() => handleDisconnectSocial("twitter_url")}
                    class="px-3 py-2 mb-3 mr-3 text-sm font-medium text-center text-gray-900 bg-white border border-gray-300 rounded-lg hover:bg-gray-100 focus:ring-4 focus:ring-primary-300 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700"
                  >
                    Disconnect
                  </button>
                {:else}
                  <button
                    type="button"
                    on:click={() => handleConnectSocial("twitter_url")}
                    class="px-3 py-2 mb-3 mr-3 text-sm font-medium text-center text-white rounded-lg bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 dark:bg-primary-1 dark:hover:bg-primary-700 dark:focus:ring-primary-800"
                  >
                    Connect
                  </button>
          ...
          <li class="py-4">
            <div class="flex items-center space-x-4">
              ...
              <div class="flex-1 min-w-0">
                ...
                {:else if isEditable["github_url"]}
                  <input
                    type="text"
                    name="github_url"
                    id="github_url"
                    class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                    bind:value={socialLink.github_url}
              ...
              <div
                class="inline-flex items-center {isEditable.github_url
                  ? 'hidden'
                  : ''}"
              >
                {#if userProfile.github_url}
                  <button
                    type="button"
                    on:click={() => handleDisconnectSocial("github_url")}
                    class="px-3 py-2 mb-3 mr-3 text-sm font-medium text-center text-gray-900 bg-white border border-gray-300 rounded-lg hover:bg-gray-100 focus:ring-4 focus:ring-primary-300 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700"
                  >
                    Disconnect
                  </button>
                {:else}
                  <button
                    type="button"
                    on:click={() => handleConnectSocial("github_url")}
                    class="px-3 py-2 mb-3 mr-3 text-sm font-medium text-center text-white rounded-lg bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 dark:bg-primary-1 dark:hover:bg-primary-700 dark:focus:ring-primary-800"
                  >
                    Connect
                  </button>
          ...
          <li class="pt-4 pb-6">
            <div class="flex items-center space-x-4">
              ...
              <div class="flex-1 min-w-0">
                ...
                {#if userProfile.linkdin_url}
                  <a
                    href={userProfile.linkdin_url}
                    class="block text-sm font-normal truncate text-primary-700 hover:underline dark:text-primary-1"
                  >
                    {userProfile.linkdin_url}
                  </a>
                {:else if isEditable["linkdin_url"]}
                  <input
                    type="text"
                    name="linkdin_url"
                    id="linkdin_url"
                    class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                    bind:value={socialLink.linkdin_url}
              ...
              <div
                class="inline-flex items-center {isEditable.linkdin_url
                  ? 'hidden'
                  : ''}"
              >
                {#if userProfile.linkdin_url}
                  <button
                    type="button"
                    on:click={() => handleDisconnectSocial("linkdin_url")}
                    class="px-3 py-2 mb-3 mr-3 text-sm font-medium text-center text-gray-900 bg-white border border-gray-300 rounded-lg hover:bg-gray-100 focus:ring-4 focus:ring-primary-300 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700"
                  >
                    Disconnect
                  </button>
                {:else}
                  <button
                    type="button"
                    on:click={() => handleConnectSocial("linkdin_url")}
                    class="px-3 py-2 mb-3 mr-3 text-sm font-medium text-center text-white rounded-lg bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 dark:bg-primary-1 dark:hover:bg-primary-700 dark:focus:ring-primary-800"
                  >
                    Connect
                  </button>
            ...
            >
              <div class="flex space-x-4 xl:mb-4 2xl:mb-0">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <p
                    class="mb-1 text-sm font-normal truncate text-primary-700 dark:text-primary-1"
                  >
                    New York, USA
                  </p>
                  <p
...
<div class="grid grid-cols-1 px-4 xl:grid-cols-2 xl:gap-4">
  <div
    class="p-4 mb-4 bg-white border border-gray-200 rounded-lg shadow-sm dark:border-gray-700 sm:p-6 dark:bg-primary-3 xl:mb-0"
  >
    <div class="flow-root">
